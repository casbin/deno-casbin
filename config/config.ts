...
export class Config implements ConfigInterface {
  ...
  /**
     * fromFile create an empty configuration representation from file.
     *
     * @param path the path of the model file.
     * @return the constructor of Config.
     */
  public static fromFile(path: string): Config {
    const config = new Config();
    config.parse(readFile(path).toString());
    return config;
  }

  /**
     * fromText create an empty configuration representation from text.
     *
     * @param text the model text.
     * @return the constructor of Config.
     */
  public static fromText(text: string): Config {
    const config = new Config();
    config.parse(text);
    return config;
  }
