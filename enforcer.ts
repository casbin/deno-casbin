...
import { ManagementEnforcer } from "./management_enforcer.ts";
import { Model, newModel } from "./model/mod.ts";
import { Adapter, FileAdapter, StringAdapter } from "./persist/mod.ts";
import { getLogger } from "./logger.ts";
import { arrayRemoveDuplicates } from "./util/mod.ts";
...
export class Enforcer extends ManagementEnforcer {
   ...
   * @param adapter current adapter instance
   */
  public async initWithModelAndAdapter(
    m: Model,
    adapter?: Adapter,
  ): Promise<void> {
    if (adapter) {
   ...
   */
  public async getImplicitUsersForPermission(
    ...permission: string[]
  ): Promise<string[]> {
    const res: string[] = [];
    const policySubjects = await this.getAllSubjects();
    const subjects = arrayRemoveDuplicates(
      [
        ...policySubjects,
