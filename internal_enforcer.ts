...
import { CoreEnforcer } from "./core_enforcer.ts";
import { BatchAdapter } from "./persist/batch_adapter.ts";
import { PolicyOp } from "./model/mod.ts";
...
export class InternalEnforcer extends CoreEnforcer {
   ...
   */
  public async addPolicyInternal(
    sec: string,
    ptype: string,
    rule: string[],
  ): Promise<boolean> {
  ...
  public async addPoliciesInternal(
    sec: string,
    ptype: string,
    rules: string[][],
  ): Promise<boolean> {
   ...
   */
  public async removePolicyInternal(
    sec: string,
    ptype: string,
    rule: string[],
  ): Promise<boolean> {
    ...
    if (sec === "g" && ok) {
      await this.buildIncrementalRoleLinks(PolicyOp.PolicyRemove, ptype, [
        rule,
      ]);
    }
    return ok;
  ...
  // removePolicies removes rules from the current policy.
  public async removePoliciesInternal(
    sec: string,
    ptype: string,
    rules: string[][],
  ): Promise<boolean> {
    ...
    if (sec === "g" && ok && effects?.length) {
      await this.buildIncrementalRoleLinks(
        PolicyOp.PolicyRemove,
        ptype,
        effects,
      );
    }
   ...
   */
  public async removeFilteredPolicyInternal(
    sec: string,
    ptype: string,
    fieldIndex: number,
    fieldValues: string[],
    ...
    const [ok, effects] = this.model.removeFilteredPolicy(
      sec,
      ptype,
      fieldIndex,
      ...fieldValues,
