...
export class Assertion {
  ...
  public async buildIncrementalRoleLinks(
    ...
    for (let rule of rules) {
      ...
      switch (op) {
        case PolicyOp.Add:
          await rm.addLink(rule[0], rule[1], ...rule.slice(2));
          break;
        case PolicyOp.Remove:
          await rm.deleteLink(rule[0], rule[1], ...rule.slice(2));
          break;
        default:
  ...
  public async buildRoleLinks(rm: RoleManager): Promise<void> {
    await this.buildIncrementalRoleLinks(rm, PolicyOp.Add, this.policy);
